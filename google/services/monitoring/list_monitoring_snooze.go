...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewMonitoringSnoozeListResource,
	}.Register()
}

var _ list.ListResource = &MonitoringSnoozeListResource{}

type MonitoringSnoozeListResource struct {
	tpgresource.ListResourceMetadata
}

func NewMonitoringSnoozeListResource() list.ListResource {
	listR := &MonitoringSnoozeListResource{}
	listR.TypeName = "google_monitoring_snooze"
	listR.SDKv2Resource = ResourceMonitoringSnooze()
...
// MonitoringSnoozeListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type MonitoringSnoozeListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *MonitoringSnoozeListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data MonitoringSnoozeListModel
...
func ListMonitoringSnoozes(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceMonitoringSnooze().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{MonitoringBasePath}}v3/projects/{{project}}/snoozes")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceMonitoringSnooze(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceMonitoringSnoozeFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
