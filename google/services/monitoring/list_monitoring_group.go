...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewMonitoringGroupListResource,
	}.Register()
}

var _ list.ListResource = &MonitoringGroupListResource{}

type MonitoringGroupListResource struct {
	tpgresource.ListResourceMetadata
}

func NewMonitoringGroupListResource() list.ListResource {
	listR := &MonitoringGroupListResource{}
	listR.TypeName = "google_monitoring_group"
	listR.SDKv2Resource = ResourceMonitoringGroup()
...
// MonitoringGroupListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type MonitoringGroupListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *MonitoringGroupListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data MonitoringGroupListModel
...
func ListMonitoringGroups(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceMonitoringGroup().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{MonitoringBasePath}}v3/projects/{{project}}/groups")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceMonitoringGroup(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceMonitoringGroupFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
