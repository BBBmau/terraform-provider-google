...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewMonitoringNotificationChannelListResource,
	}.Register()
}

var _ list.ListResource = &MonitoringNotificationChannelListResource{}

type MonitoringNotificationChannelListResource struct {
	tpgresource.ListResourceMetadata
}

func NewMonitoringNotificationChannelListResource() list.ListResource {
	listR := &MonitoringNotificationChannelListResource{}
	listR.TypeName = "google_monitoring_notification_channel"
	listR.SDKv2Resource = ResourceMonitoringNotificationChannel()
...
// MonitoringNotificationChannelListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type MonitoringNotificationChannelListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *MonitoringNotificationChannelListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data MonitoringNotificationChannelListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListMonitoringNotificationChannels(
			listR.Client,
			project,
...
func ListMonitoringNotificationChannels(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceMonitoringNotificationChannel().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{MonitoringBasePath}}v3/projects/{{project}}/notificationChannels")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceMonitoringNotificationChannel(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "notificationChannels",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if res == nil {
				return fmt.Errorf("error decoding MonitoringNotificationChannel from list response")
			}
			...
			if err = ResourceMonitoringNotificationChannelFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
