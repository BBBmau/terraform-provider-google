...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewMonitoringUptimeCheckConfigListResource,
	}.Register()
}

var _ list.ListResource = &MonitoringUptimeCheckConfigListResource{}

type MonitoringUptimeCheckConfigListResource struct {
	tpgresource.ListResourceMetadata
}

func NewMonitoringUptimeCheckConfigListResource() list.ListResource {
	listR := &MonitoringUptimeCheckConfigListResource{}
	listR.TypeName = "google_monitoring_uptime_check_config"
	listR.SDKv2Resource = ResourceMonitoringUptimeCheckConfig()
...
// MonitoringUptimeCheckConfigListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type MonitoringUptimeCheckConfigListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *MonitoringUptimeCheckConfigListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data MonitoringUptimeCheckConfigListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListMonitoringUptimeCheckConfigs(
			listR.Client,
			project,
...
func ListMonitoringUptimeCheckConfigs(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceMonitoringUptimeCheckConfig().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{MonitoringBasePath}}v3/projects/{{project}}/uptimeCheckConfigs")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceMonitoringUptimeCheckConfig(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "uptimeCheckConfigs",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceMonitoringUptimeCheckConfigFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
