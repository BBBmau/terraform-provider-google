...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewMonitoringServiceListResource,
	}.Register()
}

var _ list.ListResource = &MonitoringServiceListResource{}

type MonitoringServiceListResource struct {
	tpgresource.ListResourceMetadata
}

func NewMonitoringServiceListResource() list.ListResource {
	listR := &MonitoringServiceListResource{}
	listR.TypeName = "google_monitoring_custom_service"
	listR.SDKv2Resource = ResourceMonitoringService()
...
// MonitoringServiceListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type MonitoringServiceListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *MonitoringServiceListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data MonitoringServiceListModel
...
func ListMonitoringServices(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceMonitoringService().Data(&terraform.InstanceState{})
	if project != "" {
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceMonitoringService(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceMonitoringServiceFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
