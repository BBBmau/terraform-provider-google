...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewMonitoringGenericServiceListResource,
	}.Register()
}

var _ list.ListResource = &MonitoringGenericServiceListResource{}

type MonitoringGenericServiceListResource struct {
	tpgresource.ListResourceMetadata
}

func NewMonitoringGenericServiceListResource() list.ListResource {
	listR := &MonitoringGenericServiceListResource{}
	listR.TypeName = "google_monitoring_service"
	listR.SDKv2Resource = ResourceMonitoringGenericService()
...
// MonitoringGenericServiceListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type MonitoringGenericServiceListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *MonitoringGenericServiceListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data MonitoringGenericServiceListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListMonitoringGenericServices(
			listR.Client,
			project,
			func(rd *schema.ResourceData) error {
				result := listReq.NewListResult(ctx)

				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "service_id"); err != nil {
					return err
				}
...
func ListMonitoringGenericServices(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceMonitoringGenericService().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{MonitoringBasePath}}v3/projects/{{project}}/services")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceMonitoringGenericService(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			var err error
			if v, ok := res["serviceId"]; ok && v != nil {
				if err := d.Set("service_id", v); err != nil {
					return fmt.Errorf("error setting service_id: %w", err)
				}
			}
			if err = ResourceMonitoringGenericServiceFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
