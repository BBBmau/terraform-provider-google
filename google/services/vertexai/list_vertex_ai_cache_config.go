...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewVertexAICacheConfigListResource,
	}.Register()
}

var _ list.ListResource = &VertexAICacheConfigListResource{}

type VertexAICacheConfigListResource struct {
	tpgresource.ListResourceMetadata
}

func NewVertexAICacheConfigListResource() list.ListResource {
	listR := &VertexAICacheConfigListResource{}
	listR.TypeName = "google_vertex_ai_cache_config"
	listR.SDKv2Resource = ResourceVertexAICacheConfig()
...
// VertexAICacheConfigListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type VertexAICacheConfigListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *VertexAICacheConfigListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data VertexAICacheConfigListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListVertexAICacheConfigs(
			listR.Client,
			project,
			func(rd *schema.ResourceData) error {
				result := listReq.NewListResult(ctx)

				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "project"); err != nil {
					return err
				}
...
func ListVertexAICacheConfigs(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceVertexAICacheConfig().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{VertexAIBasePath}}projects/{{project}}/cacheConfig")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceVertexAICacheConfig(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "cacheConfigs",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			headers := make(http.Header)
			var err error
			if err = ResourceVertexAICacheConfigFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
