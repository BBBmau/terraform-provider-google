...
func TestAccApigeeEnvironmentKeyvaluemapsEntriesListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_apigee_environment_keyvaluemaps_entries.apigee_environment_keyvaluemaps_entries",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"env_keyvaluemap_id": "google_apigee_environment_keyvaluemaps_entries.apigee_environment_keyvaluemaps_entries",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccApigeeEnvironmentKeyvaluemapsEntries_apigeeEnvironmentKeyvaluemapsEntriesTestExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_apigee_environment_keyvaluemaps_entries.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_apigee_environment_keyvaluemaps_entries.list_query",
...
func testAccApigeeEnvironmentKeyvaluemapsEntries_apigeeEnvironmentKeyvaluemapsEntriesTestExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "env_keyvaluemap_id" { type = string }
list "google_apigee_environment_keyvaluemaps_entries" "list_query" {
    provider = google
	limit = 10000
    config {
