...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Apigee",
		Func:        NewApigeeSyncAuthorizationListResource,
	}.Register()
}

var _ list.ListResource = &ApigeeSyncAuthorizationListResource{}

type ApigeeSyncAuthorizationListResource struct {
	tpgresource.ListResourceMetadata
}

func NewApigeeSyncAuthorizationListResource() list.ListResource {
	listR := &ApigeeSyncAuthorizationListResource{}
	listR.TypeName = "google_apigee_sync_authorization"
	listR.SDKv2Resource = ResourceApigeeSyncAuthorization()
	listR.ListConfigFields = []tpgresource.ListConfigField{}
	return listR
}

// ApigeeSyncAuthorizationListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ApigeeSyncAuthorizationListModel struct {
}

func (listR *ApigeeSyncAuthorizationListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ApigeeSyncAuthorizationListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListApigeeSyncAuthorizations(
			listR.Client,
...
func ListApigeeSyncAuthorizations(config *transport_tpg.Config,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceApigeeSyncAuthorization().Data(&terraform.InstanceState{})
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceApigeeSyncAuthorization(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceApigeeSyncAuthorizationFlatten(d, config, res, config, userAgent, billingProject, url, headers); err != nil {
				return err
			}
