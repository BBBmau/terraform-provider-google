...
func TestAccNetworkConnectivitySpokeListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					listScope.Capture(map[string]string{
						"location": "google_network_connectivity_spoke.primary",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccNetworkConnectivitySpoke_networkConnectivitySpokeLinkedVpcNetworkBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_network_connectivity_spoke.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_network_connectivity_spoke.list_query",
...
func testAccNetworkConnectivitySpoke_networkConnectivitySpokeLinkedVpcNetworkBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_network_connectivity_spoke" "list_query" {
    provider = google
	limit = 10000
    config {
