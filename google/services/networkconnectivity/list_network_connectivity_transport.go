...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkConnectivityTransportListResource,
	}.Register()
}

var _ list.ListResource = &NetworkConnectivityTransportListResource{}

type NetworkConnectivityTransportListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkConnectivityTransportListResource() list.ListResource {
	listR := &NetworkConnectivityTransportListResource{}
	listR.TypeName = "google_network_connectivity_transport"
	listR.SDKv2Resource = ResourceNetworkConnectivityTransport()
...
// NetworkConnectivityTransportListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkConnectivityTransportListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *NetworkConnectivityTransportListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkConnectivityTransportListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkConnectivityTransports(
			listR.Client,
			region,
			project,
...
func ListNetworkConnectivityTransports(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkConnectivityTransport().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkConnectivityBasePath}}projects/{{project}}/locations/{{region}}/transports")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkConnectivityTransport(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkConnectivityTransportFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
