...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkConnectivityRegionalEndpointListResource,
	}.Register()
}

var _ list.ListResource = &NetworkConnectivityRegionalEndpointListResource{}

type NetworkConnectivityRegionalEndpointListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkConnectivityRegionalEndpointListResource() list.ListResource {
	listR := &NetworkConnectivityRegionalEndpointListResource{}
	listR.TypeName = "google_network_connectivity_regional_endpoint"
	listR.SDKv2Resource = ResourceNetworkConnectivityRegionalEndpoint()
...
// NetworkConnectivityRegionalEndpointListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkConnectivityRegionalEndpointListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetworkConnectivityRegionalEndpointListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkConnectivityRegionalEndpointListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkConnectivityRegionalEndpoints(
			listR.Client,
			location,
			project,
...
func ListNetworkConnectivityRegionalEndpoints(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkConnectivityRegionalEndpoint().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkConnectivityBasePath}}projects/{{project}}/locations/{{location}}/regionalEndpoints")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkConnectivityRegionalEndpoint(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "regionalEndpoints",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkConnectivityRegionalEndpointFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
