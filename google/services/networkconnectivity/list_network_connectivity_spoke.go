...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkConnectivitySpokeListResource,
	}.Register()
}

var _ list.ListResource = &NetworkConnectivitySpokeListResource{}

type NetworkConnectivitySpokeListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkConnectivitySpokeListResource() list.ListResource {
	listR := &NetworkConnectivitySpokeListResource{}
	listR.TypeName = "google_network_connectivity_spoke"
	listR.SDKv2Resource = ResourceNetworkConnectivitySpoke()
...
// NetworkConnectivitySpokeListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkConnectivitySpokeListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetworkConnectivitySpokeListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkConnectivitySpokeListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkConnectivitySpokes(
			listR.Client,
			location,
			project,
...
func ListNetworkConnectivitySpokes(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkConnectivitySpoke().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVarsForId(resourceData, config, "{{NetworkConnectivityBasePath}}projects/{{project}}/locations/{{location}}/spokes")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkConnectivitySpoke(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkConnectivitySpokeFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
