...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkConnectivityPolicyBasedRouteListResource,
	}.Register()
}

var _ list.ListResource = &NetworkConnectivityPolicyBasedRouteListResource{}

type NetworkConnectivityPolicyBasedRouteListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkConnectivityPolicyBasedRouteListResource() list.ListResource {
	listR := &NetworkConnectivityPolicyBasedRouteListResource{}
	listR.TypeName = "google_network_connectivity_policy_based_route"
	listR.SDKv2Resource = ResourceNetworkConnectivityPolicyBasedRoute()
...
// NetworkConnectivityPolicyBasedRouteListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkConnectivityPolicyBasedRouteListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *NetworkConnectivityPolicyBasedRouteListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkConnectivityPolicyBasedRouteListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkConnectivityPolicyBasedRoutes(
			listR.Client,
			project,
...
func ListNetworkConnectivityPolicyBasedRoutes(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkConnectivityPolicyBasedRoute().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkConnectivityBasePath}}projects/{{project}}/locations/global/policyBasedRoutes")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkConnectivityPolicyBasedRoute(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "policyBasedRoutes",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkConnectivityPolicyBasedRouteFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
