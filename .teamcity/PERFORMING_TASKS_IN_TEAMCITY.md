...
The **Global Sweepers** project contains a **Nightly Sweeper Gate** plus separate **Project Sweeper** and **Folder Sweeper** builds. The gate starts after the GA nightly composite and snapshot-depends on both GA and Beta Service Sweepers, ensuring both provider composites and their cleanup builds have completed before global cleanup. Project and Folder Sweepers use unfiltered finish-build triggers watching the gate (the gate has no VCS branch), without their own snapshot dependencies, so both can be manually dispatched without starting the nightly test chain.
