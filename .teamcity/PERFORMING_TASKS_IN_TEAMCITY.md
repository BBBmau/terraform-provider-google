...
The **Global Sweepers** project contains a **Nightly Sweeper Gate** plus separate **Project Sweeper** and **Folder Sweeper** builds. The gate starts after the GA Service Sweeper and snapshot-depends on the Beta Service Sweeper. Since the GA sweeper is the gate's finish-build trigger, it is not also declared as a snapshot dependency; this avoids scheduling a duplicate GA sweeper. Project and Folder Sweepers use a dedicated VCS root whose default branch is `refs/heads/nightly-test`, so automatic and manual runs sweep nightly-test resources. They use unfiltered finish-build triggers watching the gate (the gate has no VCS branch) and no snapshot dependencies, so both can be manually dispatched without starting the nightly test chain.
