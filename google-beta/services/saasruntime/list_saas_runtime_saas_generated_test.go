...
func TestAccSaasRuntimeSaasListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_saas_runtime_saas.example",
						[]string{
							"name",
							"saas_id",
						},
					...
					listScope.Capture(map[string]string{
						"location": "google_saas_runtime_saas.example",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccSaasRuntimeSaas_saasRuntimeSaasBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_saas_runtime_saas.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_saas_runtime_saas.list_query",
...
func testAccSaasRuntimeSaas_saasRuntimeSaasBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_saas_runtime_saas" "list_query" {
