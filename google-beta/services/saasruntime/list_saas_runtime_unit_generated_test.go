...
func TestAccSaasRuntimeUnitListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_saas_runtime_unit.example",
						[]string{
							"name",
							"unit_id",
						},
					...
					listScope.Capture(map[string]string{
						"location": "google_saas_runtime_unit.example",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccSaasRuntimeUnit_saasRuntimeUnitBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_saas_runtime_unit.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_saas_runtime_unit.list_query",
...
func testAccSaasRuntimeUnit_saasRuntimeUnitBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_saas_runtime_unit" "list_query" {
