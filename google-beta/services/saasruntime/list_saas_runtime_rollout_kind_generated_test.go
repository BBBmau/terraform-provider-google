...
func TestAccSaasRuntimeRolloutKindListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_saas_runtime_rollout_kind.example",
						[]string{
							"name",
							"rollout_kind_id",
						},
					...
					listScope.Capture(map[string]string{
						"location": "google_saas_runtime_rollout_kind.example",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccSaasRuntimeRolloutKind_saasRuntimeRolloutKindBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_saas_runtime_rollout_kind.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_saas_runtime_rollout_kind.list_query",
...
func testAccSaasRuntimeRolloutKind_saasRuntimeRolloutKindBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_saas_runtime_rollout_kind" "list_query" {
